...
| [src/okhst.ts](src/okhst.ts) | The OKHST tone layer. `REF_EPS`, tone↔lightness transfers (`toTone`/`fromTone`, `toneFromY`/`yFromTone`), OKHST↔OKHSL conversions, `variantToOkhsl` (tone→lightness at render), `normalizeToneWindow` (`[lo,hi]` / `{lo,hi,eps}` / `false` → `{lo,hi,eps}`), `mapToneForScheme` (scheme inversion + window remap, HC bypass), `saturationCeiling` (cusp-anchored extreme taper keyed on rendered `l`/`h`, with fixed `W_DARK`/`W_LIGHT` shoulders), and `schemeToneRange` for the solver. Replaces the old Möbius `scheme-mapping.ts`. |
